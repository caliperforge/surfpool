...
pub trait SurfnetCheatcodes {
    ...
    ///     `confidential` path needs. Requires `token_program` to be Token-2022, and
    ///     merges onto an existing extension, so any other extension the mint carried is
    ///     kept. Without it, an existing mint's extension bytes are preserved.
    ///   - The account keeps its lamports, topped up to the rent floor of its new size.
    /// - `token_program`: Optional token program id, defaulting to an existing mint's owner,
    ///   or the SPL Token program for a new mint. Refused when it differs from the owner.
    ///
    /// ## Returns
...
impl SurfnetCheatcodes for SurfnetCheatcodesRpc {
    ...
    fn set_mint(
        ...
        let requested_token_program = match some_token_program_str {
            Some(token_program_str) => match verify_pubkey(&token_program_str) {
                Ok(res) => Some(res),
                Err(e) => return e.into(),
            },
        ...
        let new_mint_token_program =
            requested_token_program.unwrap_or_else(spl_token_interface::id);
        ...
        Box::pin(async move {
                ...
                .get_account(
                    ...
                    Some(Box::new(move |_| {
                        GetAccountResult::FoundAccount(
                            ...
                            Account {
                                lamports: 0,
                                owner: new_mint_token_program,
                                executable: false,
                                rent_epoch: 0,
                                data: MintAccount::new(&new_mint_token_program).pack_into_vec(),
                            },
                            AccountSource::Generated,
                        )
                    })),
                ...
                && token_program_id != spl_token_2022_interface::id()
            {
                return Err(Error::invalid_params(format!(
                    "mint {mint} is owned by {token_program_id}, which is not a token program"
                )));
            }
...
mod tests {
    ...
    async fn test_set_mint_confidential_requires_token_2022() {
        ...
        for token_program in [None, Some(spl_token_interface::id().to_string())] {
            ...
            let account = client.context.svm_locker.with_svm_reader(|svm_reader| {
                svm_reader.inner.get_account(&mint.pubkey()).unwrap()
            });
    ...
    const TRANSFER_FEE_MINT_LAMPORTS: u64 = 10_000_000_000;

    /// Seeds a Token-2022 mint carrying a transfer-fee config and more lamports
    /// than its rent floor, and returns its data length.
    fn set_transfer_fee_mint(client: &TestSetup<SurfnetCheatcodesRpc>, mint: &Pubkey) -> usize {
        use spl_token_2022_interface::extension::{
        ...
        set_account(
            ...
            mint,
            &Account {
                lamports: TRANSFER_FEE_MINT_LAMPORTS,
                data,
    ...
    async fn test_set_mint_merges_partial_confidential_update() {
        ...
        let ext = state
            .get_extension::<ConfidentialTransferMint>()
            .expect("confidential mint extension should be present");
        assert_eq!(bytes_of(&ext.authority), new_authority.pubkey().as_ref());
        assert_eq!(bytes_of(&ext.auditor_elgamal_pubkey), bytes_of(&auditor));
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_set_mint_null_removes_auditor_and_authority() {
        use bytemuck::bytes_of;
        use solana_zk_sdk::encryption::elgamal::ElGamalKeypair;
        use solana_zk_sdk_pod::encryption::elgamal::PodElGamalPubkey;
        use spl_token_2022_interface::extension::{
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_set_mint_refuses_an_owner_that_is_not_a_token_program() {
        let client = TestSetup::new(SurfnetCheatcodesRpc::empty());
        let mint = Keypair::new();
        set_account(
            &client,
            ...
            &Account {
                lamports: TRANSFER_FEE_MINT_LAMPORTS,
                data: MintAccount::new(&spl_token_interface::id()).pack_into_vec(),
        ...
        assert_eq!(MintAccount::unpack(&account.data).unwrap().decimals(), 0);
    }
    ...
    async fn test_set_mint_refuses_a_token_program_other_than_the_owner() {
        ...
        let error = client
            ...
            .set_mint(
                ...
                Some(spl_token_interface::id().to_string()),
            )
            .await
            .expect_err("a token program other than the mint's owner should be refused");
        assert_eq!(error.code, jsonrpc_core::ErrorCode::InvalidParams);
        assert!(error.message.contains("owned by"), "got: {}", error.message);
