...
#[cfg_attr(feature = "postgres", test_case(TestType::postgres(); "with postgres db"))]
#[cfg_attr(feature = "ignore_tests_ci", ignore = "flaky CI tests")]
fn test_clock_mode_expiry_does_not_produce_extra_block(test_type: TestType) {
    // Regression test for issue #712 (clock-mode wall/chain time drift).
    //
    // In `BlockProductionMode::Clock`, each `slot_time` sleep in the clock
    // runloop is expected to produce exactly one block. Once every 75
    // iterations the loop also emits `ClockEvent::ExpireBlockHash`; because
    // `start_block_production_runloop` unconditionally sets
    // `do_produce_block = true` for that event, it drives a second
    // `confirm_current_block()` in the same wall interval — one extra block
    // per 75-tick expiry cycle.
    //
    // The assertion pins the block-count mechanism, not a wall-clock drift
    // percentage: `std::thread::sleep` is bounded below (it never wakes
    // early), so any two tick-driven blocks are at least `slot_time` apart
    // in wall time. Two block-production events landing within a small
    // fraction of `slot_time` therefore means the runloop produced two
    // blocks for a single wall-clock wake, which is exactly the mechanism
    // this test pins.
    const BLOCKHASH_SLOT_TTL_TICKS: u64 = 75;
    let slot_time_ms: u64 = 100;
    let (_svm_locker, _simnet_cmd_tx, simnet_events_rx) =
        boot_simnet(BlockProductionMode::Clock, Some(slot_time_ms), test_type);

    // One full expiry cycle at slot_time=100 is 7.5 s wall; add ~1.5 s of
    // slop margin so a slower host still crosses the threshold.
    let observation_window =
        Duration::from_millis(BLOCKHASH_SLOT_TTL_TICKS * slot_time_ms + 1500);
    let deadline = std::time::Instant::now() + observation_window;

    let mut block_arrival_times: Vec<std::time::Instant> = Vec::new();
    while std::time::Instant::now() < deadline {
        let remaining = deadline.saturating_duration_since(std::time::Instant::now());
        match simnet_events_rx.recv_timeout(remaining) {
            Ok(SimnetEvent::SystemClockUpdated(_)) => {
                block_arrival_times.push(std::time::Instant::now());
            }
            Ok(_) => {}
            Err(_) => break,
        }
    ...
    assert!(
        block_arrival_times.len() > BLOCKHASH_SLOT_TTL_TICKS as usize,
        "did not observe a full expiry cycle: got {} blocks in {} ms wall, expected > {}",
        block_arrival_times.len(),
        observation_window.as_millis(),
        BLOCKHASH_SLOT_TTL_TICKS
    );

    // Any two consecutive block-production events closer than
    // `slot_time_ms / 2` in wall time must have come from a single
    // wall-clock wake in the clock runloop.
    let min_expected_gap = Duration::from_millis(slot_time_ms / 2);
    let doubled_wake_pairs: Vec<(usize, u128)> = (1..block_arrival_times.len())
        .filter_map(|i| {
            let gap = block_arrival_times[i].duration_since(block_arrival_times[i - 1]);
            if gap < min_expected_gap {
                Some((i, gap.as_millis()))
            } else {
                None
            }
        })
    ...
    assert!(
        doubled_wake_pairs.is_empty(),
        "clock runloop produced multiple blocks for a single wall-clock wake at [(block_index, \
         gap_ms), ...] = {:?} (observed {} blocks in {} ms wall; two consecutive \
         block-production events within {} ms of each other means an `ExpireBlockHash` event \
         drove a second `confirm_current_block()` on top of the Tick-driven one — issue #712)",
        doubled_wake_pairs,
        block_arrival_times.len(),
        observation_window.as_millis(),
        min_expected_gap.as_millis()
    );
}
