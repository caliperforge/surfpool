...
///
/// A party-to-party confidential `Transfer` is **not covered by this change**, and no
/// test here establishes that one completes. The blocker is a version skew: proof
/// *generation* resolves to `solana-zk-sdk` 4.0.0, pinned through
/// `spl-token-2022-interface` -> `spl-pod`, while the proof *verifier* the runtime
/// runs is `solana-zk-elgamal-proof-program` 4.1.2, pulled in by `litesvm` ->
/// `solana-builtins` and built against `solana-zk-sdk` 5.0.1. The two majors derive
/// different challenge scalars, so the program rejects the proof with
/// `proof verification failed: SigmaProof(Equality, AlgebraicRelation)`. Treat that
/// path as uncovered, not as working. It becomes testable once `litesvm` links a 4.x
/// proof program, or `spl-token-2022-interface` moves to the 5.x proof stack. The
/// proof-construction plumbing for it is recoverable from commit
/// 9eba2d8aa228a6313ab95c1eba5407ff023cddfd.
#[test_case(TestType::sqlite(); "with on-disk sqlite db")]
#[test_case(TestType::in_memory(); "with in-memory sqlite db")]
#[test_case(TestType::no_db(); "with no db")]
#[cfg_attr(feature = "postgres", test_case(TestType::postgres(); "with postgres db"))]
