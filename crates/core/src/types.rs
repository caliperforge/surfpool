...
///
/// Backs the `surfnet_setMint` cheatcode. `original` is the mint's current data:
/// its other extensions are kept, and a field `conf` omits keeps its current
/// value, which is null (auto-approve true) when the extension is first written.
pub fn build_confidential_mint_data(
    ...
    let authority = conf
        .authority
        .as_ref()
        .map(|key| key.as_deref().map(Pubkey::from_str).transpose())
        .transpose()
        ...
        .map(|key| {
            key.as_deref()
                .map(|key| {
                    ...
                    ElGamalPubkey::try_from(bytes.as_slice()).map_err(|e| {
                        format!("auditorElgamalPubkey: invalid ElGamal public key ({e})")
                    })
                })
                .transpose()
        })
    ...
    let base_len = spl_token_2022_interface::state::Mint::LEN;
    let mint_len = StateWithExtensions::<spl_token_2022_interface::state::Mint>::unpack(original)
        .and_then(|state| state.try_get_new_account_len::<ConfidentialTransferMint>())
        .map_err(|e| format!("failed to size confidential mint: {e}"))?
        .max(original.len());

    let mut buffer = vec![0u8; mint_len];
    buffer[base_len..original.len()].copy_from_slice(&original[base_len..]);
    // Token-2022 pads a mint to `Account::LEN` (165 bytes), so its account-type byte sits there too.
    // `unpack_uninitialized` refuses a buffer whose account-type byte is already set.
    ...
    if state.get_extension::<ConfidentialTransferMint>().is_err() {
        state
    ...
    let ct = state
        ...
        .map_err(|e| format!("failed to read confidential mint extension: {e}"))?;
    if let Some(authority) = authority {
        ct.authority = authority.map(Into::into).unwrap_or_default();
    }
    if let Some(auto_approve) = conf.auto_approve_new_accounts {
        ct.auto_approve_new_accounts = auto_approve.into();
    }
    if let Some(key) = auditor_elgamal_pubkey {
        ct.auditor_elgamal_pubkey = key
...
mod confidential_mint_tests {
    ...
    fn an_auditor_key_that_is_not_elgamal_material_is_refused() {
        ...
        let original = MintAccount::SplToken2022(base).pack_into_vec();
        ...
        let conf = ConfidentialTransferMintUpdate {
            auditor_elgamal_pubkey: Some(Some(bs58::encode(bytes_of(&auditor)).into_string())),
            ..Default::default()
        };
        build_confidential_mint_data(&original, &base, &conf)
            .expect("a real ElGamal pubkey is accepted");
    }
}
